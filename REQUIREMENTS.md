...
| FR-4.29 | **A group member beginning `~/` resolves to the running user's home directory when the rule file is read.** A shipped rule set otherwise carries the absolute paths of whoever wrote it, and a protected-path group is exactly where those appear. **This is not the expansion FR-5.9 and `nothing-is-expanded` forbid**: that is about a word in the command under judgement, written by the subject, where a silently empty `$HOME` would have qwark reason about `rm -rf /x` while the shell acts on `rm -rf /home/user/x`. This is qwark's own configuration, resolved once at load. **A home directory that cannot be determined is a refusal to load, not a guess**, because resolving `~/bin/` to `/bin/` would widen a protected group to every user on the host, which is that same failure reached from the configuration side. Only a leading `~/` is touched; a tilde elsewhere is left as written. | [D] |
