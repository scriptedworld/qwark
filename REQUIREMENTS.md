...
| FR-4.3 | Redirections, substitutions, pipes and logical concatenation can be forbidden as one property: that the command's effect is determined by its own text. | [A] |
...
| FR-10.6a | Those two fields are present only for a subagent, so a main-session call carries neither. **This does not need solving inside qwark: an external process chooses which rule files are used for a given agent** (owner, 2026-08-20), and the rule files are already named on the command line. qwark is told which policy applies rather than working it out, which also removes the question of whether the subject could influence the answer. | [A] |
