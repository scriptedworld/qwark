...
The predecessor was `claude/hooks/archive-guard.sh` in `~/.projects/dotfiles`,
at commit `0e6ea10`: a `grep -E` over the raw command string, since retired. Its
own header records the failure that retired it:
